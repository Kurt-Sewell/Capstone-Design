# xml_logger.py
# Saves samples to XML inside a subfolder "xml files" next to your code.
# Cross-platform and atomic writes (no cross-device errors).

import os, datetime, tempfile, xml.etree.ElementTree as ET

class XMLLogger:
    def __init__(self, path, session_meta=None, rotate_daily=False, subdir_name="xml files"):
        # --- resolve final save path ---
        filename = os.path.basename(path)
        # If the caller gave no folder, default to the project folder (this file's dir)
        base_dir = os.path.dirname(os.path.abspath(path)) or os.path.dirname(__file__)
    ...
    def _write_atomic(self):
        # Write temp file in the SAME directory as final target (prevents cross-device link errors)
        ...
        try:
            ...
            # If something failed before replace, ensure tmp is gone
